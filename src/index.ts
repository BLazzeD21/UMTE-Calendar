...
const main = async () => {
  ...
  if (schedule.length) {
    console.log(
      `${colors.green}Schedule successfully retrieved!${colors.reset}`
    );
  } else {
    console.error(
      `${colors.red}Error while getting schedule. Exiting...${colors.reset}`
    );
    return;
  }

  console.log(
    `${colors.cyan}Checking if the calendar directory exists...${colors.reset}`
  );
  if (!fs.existsSync(dirname)) {
    console.log(
      `${colors.yellow}Directory does not exist. Creating it...${colors.reset}`
    );
    fs.mkdirSync(dirname);
  }

  console.log(
    `${colors.cyan}Saving schedule data to schedule.json...${colors.reset}`
  );
  fs.writeFileSync(
  ...
  console.log(
    `${colors.green}Data successfully written to schedule.json${colors.reset}`
  );
  ...
  if (calendar) {
    ...
    console.log(
      `${colors.green}Data successfully written to calendar.ics${colors.reset}`
    );
  } else {
